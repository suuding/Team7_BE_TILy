...
public class DummyEntity {
    ...
    protected Roadmap newGroupRoadmap(User creator, Category category, String name, String description, boolean isPublic, String code, boolean isRecruit, int stepNum){
        return Roadmap.builder()
                .creator(creator)
                .category(category)
                .name(name)
    ...
    protected List<Roadmap> roadmapDummyList(List<User> userListPS){
        return Arrays.asList(
                ...
                newTilyRoadmap(userListPS.get(3), Category.CATEGORY_TILY, "spring boot - 초급편", "틸리에서 제공하는 spring boot 초급자를 위한 로드맵입니다.", 20, "image.jpg"),
                newTilyRoadmap(userListPS.get(3), Category.CATEGORY_TILY, "spring boot - 중급편", "틸리에서 제공하는 spring boot 중급자를 위한 로드맵입니다.", 30 , "image.jpg"),
                newGroupRoadmap(userListPS.get(0), Category.CATEGORY_GROUP, "JAVA 입문 수업 - 생활 코딩", "생활 코딩님의 로드맵입니다!", true, "pnu1234", true, 3),
                newGroupRoadmap(userListPS.get(1), Category.CATEGORY_GROUP, "JPA 스터디", "김영한 강사님의 JPA를 공부하는 스터디 ^^", false, "ashfkc", true, 10)
        );
    }
